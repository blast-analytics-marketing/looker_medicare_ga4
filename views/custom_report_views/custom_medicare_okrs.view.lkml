    ...
    session_list_with_event_history as (
        select timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) session_date
            ...
            , events.device
              ...
              -- EDIT FOR DATA DATE RANGE --
              --where 1 = 1
              where timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) >= ((TIMESTAMP_ADD(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY), INTERVAL -750 DAY)))
...
## Dimensions
    ...
    dimension: landing_page {
      ...
      sql: (select coalesce(regexp_extract((select value.string_value from UNNEST(event_params) where key = "page_location"),r"(?:.*?[\.][^\/]*)([\/][^\?]+)"),'/')
          from UNNEST(custom_medicare_okrs.event_data) as event_history
          where event_history.sl_key = (custom_medicare_okrs.sl_key) and event_history.page_view_rank = 1 limit 1) ;;
    }
    dimension: exit_page {
      view_label: "Behavior"
      group_label: "Pages"
      ...
      sql: (select coalesce(regexp_extract((select value.string_value from UNNEST(event_params) where key = "page_location"),r"(?:.*?[\.][^\/]*)([\/][^\?]+)"),'/')
          from UNNEST(custom_medicare_okrs.event_data) as event_history
          where event_history.sl_key = (custom_medicare_okrs.sl_key) and event_history.page_view_reverse_rank = 1 limit 1) ;;
    }
  ...
  dimension: session_saw_homepage {
    type: yesno
    sql: IF('/' in (select coalesce(regexp_extract((select value.string_value from UNNEST(event_params) where key = "page_location"),r"(?:.*?[\.][^\/]*)([\/][^\?]+)"),'/')
          from UNNEST(custom_medicare_okrs.event_data) as event_history),True,False)
           ;;
    label: "Visited Home Page"
    description: "If a session visited the homepage then yes, otherwise no"
  }
